...
-- activity_per_dock and net_flow_per_dock must be null if and only if capacity is null.
...
where
    ...
    (capacity is null and (activity_per_dock is not null or net_flow_per_dock is not null))
    -- or capacity exists but a ratio is missing
    or (capacity is not null and (activity_per_dock is null or net_flow_per_dock is null))